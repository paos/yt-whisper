...
def save_to_db(data: Dict, db_path: Optional[str] = None) -> None:
    ...
    if existing:
        ...
        cursor.execute('''
        INSERT INTO videos (
            id, url, title, channel, author, upload_date, duration, description, transcription, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            data["id"],
            data["url"],
...
def get_transcript(youtube_id: str, db_path: Optional[str] = None) -> Optional[Dict]:
...
def list_transcripts(limit: int = 10, db_path: Optional[str] = None) -> list:
